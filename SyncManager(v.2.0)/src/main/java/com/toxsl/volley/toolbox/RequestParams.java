...
public class RequestParams implements Serializable {
    ...
    public HttpEntity getEntity(ResponseHandlerInterface handlerInterface) throws IOException {
        if (useJsonStreamer)
            return createJsonStreamerEntity();
        else if (!forceMultipartEntity && streamParams.isEmpty() && fileParams.isEmpty() && fileArrayParams.isEmpty())
            return createFormEntity();
        else
            return createMultipartEntity(handlerInterface);
    }
    ...
    private HttpEntity createMultipartEntity(ResponseHandlerInterface responseHandlerInterface) throws IOException {
        SimpleMultipartEntity entity = new SimpleMultipartEntity(responseHandlerInterface);
        entity.setIsRepeatable(isRepeatable);
